...
int pauseScreenX = 500;
int pauseScreenY = 500;
int pauseScreenWidth = 600;
int pauseScreenHeight = 500;
float pauseScreenCornerRadius = 50;
...
boolean playerDeathHasPlayed = false;
float missileVolume = 0.01; // 0 = mute, 1 = 100% 
float enemyDeathVolume = 0.01;
...
void textMyName() {
  ...
  textSize(12);
  noStroke();
  text("Rasmus Lausten Toftegaard", 10, 20);
}
void howToPauseTxt() {
  fill(#FEFF1F);
  textSize(25);
  text("press P to pause the game", 400, 40);
}
void spawnMissile() {
  missileDelay++;
...
void enemyControl() {
  ...
  if (numberOfEnemies == 1) {
    ...
    if (enemyY > 950) {
      ...
      if (playerDeathHasPlayed) {
      } else {
        playerDeath.play();
        playerDeathHasPlayed = true;
      }
      ...
      if (kills > 1 && kills < 10) {
        ...
        text("You only have a tiny bit left for the last achivement", 300, 600);
      } else if (kills > 9000) {
      ...
      text("Press 'Q' to quit game", 400, 750);
    }
    enemyY += enemyYSpeed;
    noStroke();
    fill(#FF0329);
...
void pauseScreen() {
  ...
  stroke(255);
  fill(#393737);
  rect(pauseScreenX, pauseScreenY, pauseScreenWidth, pauseScreenHeight, pauseScreenCornerRadius, pauseScreenCornerRadius, pauseScreenCornerRadius, pauseScreenCornerRadius);
  textSize(40);
  fill(#E51E1E);
  text("GAME IS PAUSED", 350, 400);
  textSize(30);
  text("Go take a break", 370, 600);
  text("Press P to Unpause", 370, 700);

  rectMode(CORNER);
  enemyY = 0;
}
void SoundHandler() {
  //SOUNDFILES FOUND ON http://www.classicgaming.cc/classics/space-invaders/sounds NOT HOMEMADE!!  

  bGMusic = new SoundFile(this, "spaceinvaders1.wav");
  bGMusic.loop();

  playerShoot = new SoundFile(this, "shoot.wav");
  playerShoot.amp(missileVolume);
  playerDeath = new SoundFile(this, "explosion.wav");

  enemyDeath = new SoundFile(this, "invaderkilled.wav");
  enemyDeath.amp(enemyDeathVolume);
}